...
class RealChainE2EManualTest {
    ...
    void runRealChainFlow() throws Exception {
        ...
        Wallet hotWallet = resolveValidHotWallet();
    ...
    private Wallet resolveValidHotWallet() {
        return walletRepository.findAll().stream()
                .filter(wallet -> wallet.getWalletType() == WalletType.HOT)
                .filter(wallet -> wallet.getWalletAddress() != null)
                .filter(wallet -> EVM_ADDRESS.matcher(wallet.getWalletAddress()).matches())
                .findFirst()
                .orElseGet(() -> {
                    Wallet created = createWallet(WalletType.HOT, 0L, WalletStatus.ACTIVE);
                    created.updateBalance(new BigDecimal("1000000"));
                    created.updateLastUsedAt();
