...
class NotificationRepositoryTest {
    ...
    void 수신자_알림_목록_조회() {
        notificationRepository.save(notification(1L, RecipientType.USER, false));
        notificationRepository.save(notification(1L, RecipientType.USER, false));
        notificationRepository.save(notification(2L, RecipientType.USER, false)); // 다른 수신자

        Page<Notification> result = notificationRepository
                .findByReceiverNoAndRecipientType(1L, RecipientType.USER, PageRequest.of(0, 10));
    ...
    void 다른_타입_알림_미조회() {
        ...
        assertThat(result.getContent()).allMatch(n -> n.getRecipientType() == RecipientType.USER);
    }
    ...
    void 읽지않은_알림_수_조회() {
        notificationRepository.save(notification(1L, RecipientType.USER, false));
        notificationRepository.save(notification(1L, RecipientType.USER, false));
        notificationRepository.save(notification(1L, RecipientType.USER, true)); // 읽은 알림

        long count = notificationRepository
                .countByReceiverNoAndRecipientTypeAndIsRead(1L, RecipientType.USER, false);
    ...
    void 읽은_알림_카운트_제외() {
        notificationRepository.save(notification(1L, RecipientType.USER, true));
        notificationRepository.save(notification(1L, RecipientType.USER, true));
