...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Query(value = "SELECT * FROM notification WHERE receiver_no = :receiverNo AND recipient_type = lower(:#{#recipientType.name()})",
           countQuery = "SELECT count(*) FROM notification WHERE receiver_no = :receiverNo AND recipient_type = lower(:#{#recipientType.name()})",
           nativeQuery = true)
    ...
    // 읽지 않은 알림 수
    @Query(value = "SELECT count(*) FROM notification WHERE receiver_no = :receiverNo AND recipient_type = lower(:#{#recipientType.name()}) AND is_read = :isRead",
           nativeQuery = true)
